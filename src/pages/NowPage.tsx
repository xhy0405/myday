...
const NowPage: FC = () => {
  ...
  const focusCopy = {
    yesterday: {
      ...
      footer: "昨天的线索已归位",
      checkinTitle: "昨日打卡内容",
      checkinBody: "补充昨天完成了哪些固定习惯或学习内容。",
      checkinEmptyText: "昨天还没有打卡内容。可以补上“学英语”“背单词”等项目。",
    },
    today: {
      ...
      footer: "今天的工作台已就绪",
      checkinTitle: "今日打卡内容",
      checkinBody: "添加你每天想坚持的事，比如学英语、运动或早睡。",
      checkinEmptyText: "还没有打卡内容。先添加一项，比如：学英语。",
    },
    tomorrow: {
      ...
      footer: "明天的入口已轻轻打开",
      checkinTitle: "明日打卡内容",
      checkinBody: "提前写下明天想坚持的打卡项，到当天后再勾选完成。",
      checkinEmptyText: "可以提前添加明天的打卡内容，比如：学英语。",
    },
  }[focus];
  ...
  const checkinCount = record?.dailyCheckins.length ?? 0;
  const completedCheckinCount = record?.dailyCheckins.filter((item) => item.completed).length ?? 0;
  ...
  return (
    <main className="page-shell">
      <div className="lg:hidden">
        ...
        <section className="mobile-task-flow mt-4">
          <div className="mb-4 flex items-start gap-3">
            ...
            <div className="min-w-0">
              <h2 className="text-lg font-bold text-slate-950">{focusCopy.checkinTitle}</h2>
              <p className="mt-1 text-sm leading-5 text-slate-500">
                {focusCopy.checkinBody}
              </p>
              <p className="mt-1 text-xs font-semibold text-emerald-700">
                已完成 {completedCheckinCount} / {checkinCount} 项
              </p>
            </div>
          </div>
          <DailyCheckinList
            items={record?.dailyCheckins ?? []}
            onAdd={addDailyCheckin}
            onToggle={toggleDailyCheckin}
            onUpdateTitle={updateDailyCheckinTitle}
            onDelete={deleteDailyCheckin}
            emptyText={focusCopy.checkinEmptyText}
            completionReadOnly={isPlanningTomorrow}
          />
        </section>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2 lg:items-start">
        <section className="panel panel-glow-cool panel-interactive h-full lg:col-span-2">
          <div className="flex items-start gap-3">
            <div className="icon-tile">
              <CalendarCheck className="h-5 w-5" aria-hidden />
            </div>
            <div>
              <h2 className="section-title">{focusCopy.checkinTitle}</h2>
              <p className="section-copy">{focusCopy.checkinBody}</p>
